    ...
    public function performAction()
    {
        ...
        if (!$combat->isAlive($combat->getJoueur())) {
            ...
            if ($_SESSION['is_ironman']) {
                ...
                // Mode Normal : Défaite mais pas de mort permanente
                // On relève le joueur à 1 PV pour qu'il puisse fuir ou mourir à nouveau
                $charModel = new Character();
                $charModel->findById($charId);

                // Restauration de la connexion DB si nécessaire (via __wakeup ou manuellement ici)
                // Le modèle Character gère maintenant sa connexion, mais par sécurité on instancie proprement
                $charModel->heal($charId, 1);
                ...
                echo json_encode([
                    ...
                    "playerHp" => 1,
                    "win" => false,
                    "defeat" => true, // Indicateur spécifique pour le frontend
                    "gameOver" => false,
                    "message" => "Vous avez été vaincu ! Vous reprenez conscience avec difficulté..."
                ]);
                unset($_SESSION['combat']);
                exit;
            }
