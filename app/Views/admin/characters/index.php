...
<div class="bg-slate-900 border border-slate-800 rounded-xl p-6 mb-6">
    ...
    <!-- Filters -->
    <form action="/admin/characters" method="GET" class="bg-slate-900 border border-slate-800 rounded-xl p-6 mb-6 grid grid-cols-1 md:grid-cols-5 gap-4">
        <div>
            <label class="block text-slate-400 text-sm mb-2 font-medium">Nom</label>
            <input type="text" name="name" value="<?= htmlspecialchars($_GET['name'] ?? '') ?>" class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-slate-200 text-sm focus:outline-none focus:border-indigo-500 transition-colors" placeholder="Rechercher...">
        </div>
        <div>
            <label class="block text-slate-400 text-sm mb-2 font-medium">Classe</label>
            <select name="class_id" class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-slate-200 text-sm focus:outline-none focus:border-indigo-500 transition-colors">
        ...
        <div>
            ...
            <input type="number" name="level" value="<?= htmlspecialchars($_GET['level'] ?? '') ?>" class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-slate-200 text-sm focus:outline-none focus:border-indigo-500 transition-colors" placeholder="Niveau...">
        </div>
        <div>
            <label class="block text-slate-400 text-sm mb-2 font-medium">Propriétaire</label>
            <select name="user_id" class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-slate-200 text-sm focus:outline-none focus:border-indigo-500 transition-colors">
        ...
        <div class="flex items-end">
            <button type="submit" class="w-full px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-lg transition-all transform hover:-translate-y-0.5">
                🔍 Filtrer
            </button>
        </div>
    </form>
            ...
            <tbody>
                <?php foreach ($characters as $char): ?>
                    <tr class="border-t border-slate-800 hover:bg-indigo-500/5 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <span class="inline-block bg-purple-500/20 text-purple-300 px-3 py-1 rounded-lg text-sm font-medium">
                                <?= htmlspecialchars($char['class_name']) ?>
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-block bg-green-500/20 text-green-300 px-3 py-1 rounded-lg text-sm font-medium">
                                Niv. <?= $char['level'] ?? 1 ?>
                            </span>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            <form action="/admin/characters/delete/<?= $char['id'] ?>" method="POST" class="inline" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce personnage ?');">
