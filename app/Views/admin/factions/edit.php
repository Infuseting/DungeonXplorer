...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-3xl font-bold text-gray-100">Modifier : <?= htmlspecialchars($faction['name']) ?></h1>
    </div>

    <div class="bg-gray-800 rounded-lg shadow-lg p-6 border border-gray-700">
        <form action="/admin/factions/edit/<?= $faction['id'] ?>" method="POST" class="space-y-6">
            
            <!-- Name -->
            <div>
                <label for="name" class="block text-sm font-medium text-gray-300 mb-2">Nom de la Faction</label>
                <input type="text" id="name" name="name" required value="<?= htmlspecialchars($faction['name']) ?>"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors"><?= htmlspecialchars($faction['description'] ?? '') ?></textarea>
            </div>
