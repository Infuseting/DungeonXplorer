...
<div class="card">
    ...
    <div class="flex items-center gap-4 mb-6">
        <input type="text" id="search-input" class="form-input flex-1" placeholder="🔍 Rechercher par nom..."
        ...
        <select id="role-filter" class="form-select w-auto">
            ...
            <option value="quest_giver" <?= ($roleFilter ?? '') === 'quest_giver' ? 'selected' : '' ?>>Donneur de quêtes
            </option>
    ...
    <?php if (!empty($npcs)): ?>
        <div class="grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-6 mt-6">
            <?php foreach ($npcs as $npc): ?>
                <div
                    class="bg-gray-800 border border-gray-700 rounded-xl p-6 transition-all duration-200 hover:border-indigo-500 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-black/30">
                    <div class="flex gap-4 mb-4">
                        <div
                            class="w-16 h-16 bg-gray-900 border-2 border-gray-700 rounded-lg flex items-center justify-center overflow-hidden">
                            <?php if ($npc['texture']): ?>
