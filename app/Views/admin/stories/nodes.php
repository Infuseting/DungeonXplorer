...
<div class="flex flex-col h-[calc(100vh-100px)]">
    ...
    <div class="flex justify-between items-center mb-4">
        ...
        <div class="flex gap-2">
            <button onclick="storyEditor.addNode()"
                class="bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded text-sm font-medium flex items-center gap-1">
                ➕ Ajouter Nœud
            </button>
            <button onclick="storyEditor.saveLayout()"
                class="bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded text-sm font-medium flex items-center gap-1">
    ...
    <div class="flex flex-1 gap-4 overflow-hidden">
        <!-- Canvas Area -->
        <div class="flex-1 bg-gray-900 rounded-lg border border-gray-700 relative overflow-hidden"
            id="editor-container">
            <div id="canvas-grid" class="absolute inset-0 pointer-events-none opacity-20"></div>
            <div id="nodes-layer" class="absolute inset-0 overflow-visible">
                <!-- Nodes will be rendered here by JS -->
            </div>
            <svg id="connections-layer" class="absolute inset-0 w-full h-full pointer-events-none overflow-visible">
                <!-- Connections will be rendered here -->
            </svg>
        </div>

        <!-- Sidebar / Properties Panel -->
        <div class="w-80 bg-gray-800 rounded-lg border border-gray-700 flex flex-col overflow-hidden hidden"
            id="properties-panel">
