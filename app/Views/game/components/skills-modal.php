...
<div id="skills-modal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="relative z-10 w-full h-full flex items-center justify-center p-4 lg:p-8 pointer-events-none">
        <div
            class="bg-gray-800/90 border border-gray-600 rounded-2xl shadow-2xl flex flex-col p-4 lg:p-6 max-w-6xl w-full h-[90vh] pointer-events-auto transform transition-all scale-100 overflow-hidden">
            ...
            <!-- Canvas Container -->
            <div class="relative w-full flex-grow bg-gray-900 border border-gray-700 rounded-lg overflow-hidden cursor-grab active:cursor-grabbing"
                id="skill-tree-container">
                <!-- SVG Layer for Lines -->
                <svg id="skill-connections-layer"
                ...
                <div
                    class="absolute bottom-4 right-4 bg-gray-800/80 p-2 rounded text-xs text-gray-400 pointer-events-none select-none">
        ...
        const SkillTree = {
            ...
            renderNode(skill) {
                ...
                if (skill.status === 'unlocked') {
                    classes += " bg-gray-800 border-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.3)] z-20 text-gray-100";
                    statusIcon = `<span class="material-symbols-outlined text-amber-500">check_circle</span>`;
                } else if (skill.status === 'available') {
                    classes += " bg-gray-800 border-gray-500 hover:border-amber-400 hover:scale-105 cursor-pointer z-20 text-white";
                    ...
                    classes += " bg-gray-900 border-gray-800 text-gray-500 opacity-60 z-10 grayscale";
                    statusIcon = `<span class="material-symbols-outlined text-gray-600">lock</span>`;
                }
