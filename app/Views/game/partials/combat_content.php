...
<div id="combat-scene" class="absolute inset-0 w-full h-full bg-gray-900 overflow-y-auto z-50 p-2 md:p-3"
    ...
    <div class="flex flex-col md:flex-col justify-center items-center gap-8 max-w-7xl mx-auto">
        ...
        <div
            class="w-full md:w-[70%] h-[500px] bg-gray-700 rounded-lg flex items-center justify-center drop-shadow-[0_0_5px_#a78bfa] relative overflow-hidden">
            <img src="<?= $monsterModel->getSallePath() ?>"
                class="w-full h-full object-cover absolute inset-0 text-white" id="bg" alt="Décor de salle">

            <div class="absolute top-8 flex flex-col text-justify z-10 w-full px-4">
                <div class="flex justify-between items-center mb-2">
                    <h2
                        class="text-2xl md:text-3xl font-bold text-white text-center drop-shadow-lg bg-black/50 p-2 rounded flex-1">
                        VS <?= $monsterModel->getName() ?></h2>
                    ...
                    <div class="bg-black/70 p-2 rounded-lg ml-2 w-32">
                        <div class="w-full bg-gray-700 h-4 rounded-full overflow-hidden shadow-inner">
                            <div id="monster-hp-bar" class="bg-red-500 h-full transition-all duration-500 shadow-lg"
                                style="width: 100%" data-max-hp="<?= (int) $monsterModel->getVitality() ?>"></div>
                        </div>
                    </div>
                </div>
                <div id="combat-log" class="text-center bg-black/60 p-2 rounded min-h-[60px] flex flex-col justify-end">
                </div>
            </div>
            ...
            <div class="absolute bottom-0 w-full flex justify-center h-[300px] pointer-events-none">
                <img id='monster' src="<?= $monsterModel->getImagePath() ?>" alt="Monstre"
                    class="object-contain h-full transition-opacity duration-1000 opacity-100 drop-shadow-2xl">
            </div>

            <div id="dice"
                class="absolute bottom-3 right-3 text-4xl font-bold text-violet-400 drop-shadow-[0_0_6px_#a78bfa] z-10 bg-black/50 rounded-full w-16 h-16 flex items-center justify-center">
                🎲
            </div>
        </div>

        <div
            class="w-full md:w-[70%] bg-gray-800 rounded-lg flex flex-col md:flex-row relative overflow-visible p-4 gap-4 shadow-xl border border-gray-700">
                ...
                <div class="mb-4 bg-gray-800/50 p-3 rounded-lg">
                    <div class="flex items-center justify-between mb-2">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xl font-bold text-white"
                                id="player-hp"><?= htmlspecialchars($characterModel->getCurrentHp()); ?></span>
                    ...
                    <div class="w-full bg-gray-700 h-3 rounded-full overflow-hidden">
                        <div id="player-hp-bar"
                            ...
                            style="width: <?= (($characterModel->getCurrentHp() / $characterModel->getVitality()) * 100) ?>%">
                        </div>
                    </div>
                </div>
                ...
                <ul class="text-sm grid grid-cols-2 gap-3 mx-auto w-full">
                    <li class="flex items-center gap-2 w-full bg-gray-800/50 p-2 rounded">
                        ...
                        <span class="text-lg font-bold text-white"
                            id="player-attack"><?= htmlspecialchars($characterModel->getAttaqueClass()); ?></span>
                    </li>

                    <li class="flex items-center gap-2 w-full bg-gray-800/50 p-2 rounded">
                        <span class="material-symbols-outlined text-blue-400" title="Défense du joueur">shield</span>
                        <span class="text-lg font-bold text-white"
                            id="player-defense"><?= htmlspecialchars($characterModel->getArmorClass()); ?></span>
                    </li>
                    ...
                    <li class="flex items-center gap-2 w-full bg-gray-800/50 p-2 rounded">
                        ...
                        <span class="text-lg font-bold text-white"
                            id="player-strength"><?= htmlspecialchars($characterModel->getStrength()); ?></span>
                    </li>
                    ...
                    <li class="flex items-center gap-2 w-full bg-gray-800/50 p-2 rounded">
                        ...
                        <span class="text-xl font-bold text-white"
                            id="player-intelligence"><?= htmlspecialchars($characterModel->getIntelligence()); ?></span>
                    </li>
                    ...
                    <li class="flex items-center gap-2 w-full bg-gray-800/50 p-2 rounded">
                        ...
                        <span class="text-xl font-bold text-white"
                            id="player-dexterity"><?= htmlspecialchars($characterModel->getDexterity()); ?></span>
                    </li>
                </ul>
            </div>
            ...
            <div class="w-full md:w-1/3 flex flex-col items-center justify-center relative mb-4 md:mb-0">
                ...
                <div id='character-container'
                    class="w-40 h-40 md:w-48 md:h-48 rounded-full border-4 border-violet-500 bg-gray-700 overflow-hidden flex items-center justify-center relative shadow-[0_0_15px_rgba(167,139,250,0.5)]">
                ...
                <div id="status-effects-container"
                    ...
                    <?php else: ?>
                        <?php foreach ($activeBuffs as $buff): ?>
                            <div class="px-2 py-1 bg-gray-800 border border-gray-600 rounded text-xs text-violet-300 flex items-center gap-1 shadow-sm"
            ...
            <div class="w-full md:w-1/3 p-3 bg-gray-900/50 rounded-lg flex flex-col relative">
                ...
                <div id="action-menu-main" class="flex flex-col h-full">
                    ...
                    <ul class="grid grid-cols-2 gap-3 mb-4 flex-1 content-start transition-all duration-300">
                        ...
                        <li>
                            <button id="btn-attack" disabled class="
                        ...
                        <li>
                            <button id="btn-defend" disabled class="
                            w-full px-2 py-3 text-xs md:text-sm uppercase font-bold
                        ...
                        <!-- Potions Button -->
                        <li>
                            <button id="btn-potions-menu" onclick="window.toggleActionMenu('potions')" disabled class="
                            w-full px-2 py-3 text-xs md:text-sm uppercase font-bold
                        ...
                        <li>
                            ...
                            ?>
                            <button id="btn-skills-menu" onclick="window.toggleActionMenu('skills')" disabled class="
                                w-full px-2 py-3 text-xs md:text-sm uppercase font-bold
                ...
                <div id="action-menu-potions"
                    class="hidden flex flex-col h-full absolute inset-0 bg-gray-900/95 z-20 p-3 rounded-lg backdrop-blur-sm">
                    <h3
                        class='text-yellow-400 font-semibold mb-3 text-center text-lg uppercase tracking-wider border-b border-gray-700 pb-2 flex justify-between items-center'>
                        <button onclick="window.toggleActionMenu('main')"
                                ...
                                <button onclick="usePotion(<?= $potion['id'] ?>)"
                                    ...
                                    <div class="flex items-center gap-2">
                                        <div class="text-2xl" style="height: 32px; width: 32px;">
                                            <img src="/<?= htmlspecialchars($potion['icon']) ?>" height="32" width="32"
                                                alt="🧪" />
                                        </div>
                                        <div>
                                            <div class="font-bold text-yellow-100 group-hover:text-yellow-400 text-sm">
                                                <?= htmlspecialchars($potion['name']) ?></div>
                                            <!-- Simple stat preview if available -->
                                            <div class="text-[10px] text-gray-400">Restaure HP</div>
                                        </div>
                                    </div>
                                    <div class="text-xs font-mono font-bold bg-black/50 px-2 py-1 rounded text-gray-300">
                                        x<?= $potion['count'] ?></div>
                                </button>
                            <?php endforeach; ?>
                        <?php else: ?>
                ...
                <div id="action-menu-skills"
                    class="hidden flex flex-col h-full absolute inset-0 bg-gray-900/95 z-20 p-3 rounded-lg backdrop-blur-sm">
                    <h3
                        class='text-violet-400 font-semibold mb-3 text-center text-lg uppercase tracking-wider border-b border-gray-700 pb-2 flex justify-between items-center'>
                        <button onclick="window.toggleActionMenu('main')"
                            class="text-xs text-gray-400 hover:text-white flex items-center gap-1">
                            <span class="material-symbols-outlined text-sm">arrow_back</span> Retour
                        </button>
                    ...
                    <div class="flex-1 overflow-y-auto space-y-2 pr-1">
                            ...
                            <?php foreach ($actives as $skill): ?>
                                <button onclick="sendAction('use_skill', <?= $skill['id'] ?>)"
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <div>
                                            <div class="font-bold text-violet-100 group-hover:text-violet-400 text-sm">
                                                <?= htmlspecialchars($skill['name']) ?></div>
                        ...
                        <?php else: ?>
                            <div class="text-center mt-4">
                                ...
                                <p class="text-xs text-gray-600 mt-2">Débloquez des compétences actives dans l'arbre de
                                    compétences ⚡</p>
                            </div>
                        <?php endif; ?>
                    </div>
