...
<div id="story-scene" data-story-id="<?= $story['id'] ?>" class="absolute inset-0 w-full h-full">
    ...
    <div id="top-drawer" class="fixed top-0 left-0 right-0 z-40 transition-transform duration-300 transform -translate-y-full">
        <!-- Drawer Content -->
        <div class="bg-gray-900/95 border-b border-yellow-600/30 p-6 pb-8 shadow-[0_10px_40px_rgba(0,0,0,0.8)] backdrop-blur-md">
        ...
        <div class="absolute -bottom-10 left-0 right-0 flex justify-center pointer-events-none">
            <button id="top-drawer-toggle" class="pointer-events-auto bg-gray-900/90 text-yellow-500 px-8 py-2 rounded-b-xl border-b border-x border-yellow-600/50 hover:bg-gray-800 transition-all shadow-[0_5px_15px_rgba(0,0,0,0.5)] flex items-center gap-2 group">
    ...
    <div class="fixed bottom-24 right-8 z-50">
        <button id="exit-dungeon-btn" onclick="exitDungeon()" class="hidden w-16 h-16 bg-yellow-600 hover:bg-yellow-500 text-white rounded-full shadow-lg border-2 border-yellow-400 flex items-center justify-center transition-transform hover:scale-110 active:scale-95" title="Sortir du donjon">
