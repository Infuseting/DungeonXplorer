...
class Combat
{
    ...
    {
        // Restore database connections for nested objects after deserialization
        if ($this->joueur && method_exists($this->joueur, '__wakeup')) {
            $this->joueur->__wakeup();
        }
        if ($this->boss && method_exists($this->boss, '__wakeup')) {
            $this->boss->__wakeup();
        }
    }
